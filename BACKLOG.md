...
## 2026-05-05 — Revert csam_page_size 300 → 1000 (cursor stalled at smaller values)
**Area:** `src/config_loader.py`, `config/.config.example`, `src/api_client.py::fetch_csam_assets` (docstring + inline comment), `docs/CSAM_PAGINATION.md`
**Why:** The 2026-05-04 commit `dc6e5c8` lowered `csam_page_size` default from 1000 to 300, citing Qualys's QualysETL reference implementation. Operator's pre-change logs show 50,000 unique assets pulled per query (corroborated by `Saved 1217000 host tags` — 1.2M tag rows requires ~50k unique assets each with ~24 tags, not 100 unique assets which would dedupe to ~2,400). After the change, pulls capped at 200 assets with the new cursor-stall detection firing — Qualys returned the same `lastSeenAssetId=9924156` on consecutive pages. Hypothesis confirmed by the timing: the only knob that materially changed between the working and broken state is the requested `limitResults` value. Both 1000 and 300 cause the response to clamp at ~100 assets, but the cursor's "next position" calculation apparently depends on the REQUESTED page size, not the served size — at 1000 the cursor advances normally, at 300 it stalls. This reverts the page-size change. The cursor-stall detection (also from `dc6e5c8`), the raised `max_pages=10000`, the cross-query continuation loop (commit `b88444a`), and the per-page logging (`a4c4917`) all stay — none of them caused the regression and all are useful additions for catching this kind of issue going forward. With page_size=1000 restored AND the continuation loop in place, the operator should now see the cursor advance through 50k of one query, hit the per-query cap, and the continuation loop fires for queries 2, 3, etc. until the preflight expected_count is reached.

## 2026-05-05 — CSAM lookback bucketing: guaranteed full-fleet pull
**Area:** `src/api_client.py::fetch_csam_assets` (new `filter_qql_override` param), `src/data_manager.py::_fetch_csam_with_checkpoint` (bucketed path), `src/config_loader.py` (`csam_lookback_buckets` option), `config/.config.example`, `tests/test_csam_bucketing.py`, `docs/CSAM_PAGINATION.md`
**Why:** Operator wanted guaranteed "pull all the data once" capability after the cursor-stall investigation revealed Qualys's pagination is unreliable on their tenant. The cosmetic-cursor handling (commit `e40aa27`) lets us keep paginating when the cursor is informational, but doesn't help if Qualys also returns `hasMoreRecords=0` prematurely. Bucketing sidesteps both issues by splitting the lookback window into N narrower queries — each with its own cursor lifecycle and a narrower filter that's less likely to hit a per-query asset cap. New `csam_lookback_buckets` config option (default 1 = current single-query + continuation behaviour). When > 1 AND `csam_lookback_days > 0`, the wrapper builds N date-band QQL filters (`lastCheckedIn >= LOW AND lastCheckedIn < HIGH`) and calls `fetch_csam_assets` once per bucket with `filter_qql_override` set. Bucket 0 (most recent) has no upper bound to catch just-checked-in assets. Each bucket starts with a fresh cursor (`resume_from_id=None`). Default of 1 preserves all existing behaviour. New `filter_qql_override` parameter on `fetch_csam_assets` takes priority over `lookback_days`-derived filter when set. 5 new tests cover: 3-bucket dispatch with correct date-range filters and fresh cursors per bucket, asset accumulation across buckets, `buckets=1` falls through to single-query path, `lookback_days=0` disables bucketing, and `filter_qql_override` supersedes `lookback_days` at the API client level. 168/168 green.

## 2026-05-05 — Restore 50k+ CSAM pulls: weaken cursor-stall + revert page_size to 1000
**Area:** `src/api_client.py::fetch_csam_assets` (cursor-stall logic), `src/config_loader.py`, `config/.config.example`, `tests/test_csam_pagination.py`, `docs/CSAM_PAGINATION.md`
**Why:** Operator reported pulls capping at 200 assets after the 2026-05-04 cursor-pagination work, despite pre-change logs (commit `b558917` era) clearly showing 50,000-asset pulls (corroborated by 1.2M host_tags rows, which requires ~50k unique assets × ~24 tags each, not 100 deduped). Root cause analysis identified two issues stacked:

(1) `csam_page_size` default change `1000 → 300` (commit `dc6e5c8`) — reverted. The new default 300 appeared to change Qualys's response shape on the operator's tenant. Sticking with 1000.

(2) The headline regression: a new strict cursor-stall check (also `dc6e5c8`) that aborted the loop whenever `lastSeenAssetId` failed to advance across consecutive pages. The pre-`dc6e5c8` working pulls relied on a Qualys behaviour where on some tenants `lastSeenAssetId` is **cosmetic** — the server uses an internal cursor that advances regardless of what `startFromId` we send, and the response field is informational. Operator's working logs from 2026-05-01 ran 500 pages of 100 unique assets each (50,000 total) almost certainly riding this internal cursor, since the OLD code (no cursor check) just kept calling and the server kept serving new data. Adding "cursor must advance to continue" broke that.

Fix: cursor-stall check is now two-signal — abort only when cursor stuck AND page returned 0 assets (real server stall). When cursor stuck but page has new assets (cosmetic-cursor pattern), log a one-shot WARNING and continue paginating, trusting `hasMoreRecords` for termination. The `max_pages=10000` safety cap (also from `dc6e5c8`) bounds runaway loops if both cursor and data stall.

The cross-query continuation loop (`b88444a`), per-page logging (`a4c4917`), and `max_pages=10000` raise all stay — they didn't cause the regression and remain useful additions. The combination should now restore the 50k+ pull AND let the continuation loop break past 50k once Qualys's per-query server cap kicks in. Test changes: replaced `test_cursor_stall_aborts_with_warning` (which asserted the now-broken strict behaviour) with two tests — `test_real_stall_aborts` (cursor stuck + empty page) and `test_cosmetic_cursor_keeps_paginating` (cursor stuck + new assets each page). 163/163 green.

## 2026-05-04 — CSAM cross-query continuation loop: break past the 50,000-asset *server* cap
**Area:** `src/data_manager.py::_fetch_csam_with_checkpoint`, `src/database.py` (`csam_checkpoint` schema migration: `expected_count` + `query_iteration` columns), `tests/test_csam_continuation.py`, `docs/CSAM_PAGINATION.md`
**Why:** Even with the 50,000-asset client-side cap fixed in the prior commit, production pulls were still capping at exactly 50k. Hypothesis (now confirmed by structure of the fix): Qualys silently caps a single CSAM query at ~50k assets and returns `hasMoreRecords=0` even when the tenant has more matching assets. There's no error or 429 to react to — the server says "done" cleanly. New outer loop in `_fetch_csam_with_checkpoint` watches for `total_fetched < expected_count - tolerance` after each inner-loop completion and launches another `fetch_csam_assets` call using the final `lastSeenAssetId` as a fresh `startFromId`. Tolerance is `max(100, 1% of expected)` to absorb natural fleet churn; stall guard bails after one zero-progress iteration to avoid infinite loops; preflight-None bypasses continuation since there's no reference value. Schema gained `expected_count` and `query_iteration` columns (idempotent ALTER TABLE migration) so a resume mid-continuation skips the preflight count call and logs the correct iteration number. Cold-start zero-out fix prevents a completed prior pull's `assets_pulled` count from polluting the new snapshot's `cross_query_total`. 9 new tests cover: cold start cursor omission, expected_count persistence, single-query within-tolerance success, two-query 50k-cap continuation with cursor threading, three-query stress, stall guard, preflight-unavailable single-query path, resume mid-continuation from prior cursor, completed-checkpoint fresh-start. Existing 153 tests still pass.

## 2026-05-04 — CSAM cursor pagination: break past the 50,000-asset cap
**Area:** `src/api_client.py::fetch_csam_assets`, `src/config_loader.py`, `config/.config.example`, `tests/test_csam_pagination.py`, `docs/CSAM_PAGINATION.md`
**Why:** Production CSAM pulls were terminating at exactly 50,000 assets / 500 pages on every run regardless of starting state. Root cause was the combination of `max_pages=500` (a defensive ceiling that was being treated as a normal limit) and Qualys silently clamping `limitResults=1000` requests to ~100 — so 500 × 100 = exactly 50,000. Tenant size estimated at 250k+. The fix has three parts. (1) `max_pages` default raised from 500 to 10,000 and reframed as a safety guard against runaway loops, with a loud WARNING when hit. The loop is now driven STRICTLY by `hasMoreRecords == 0` per the documented Qualys cursor contract — no count-based termination. (2) `csam_page_size` default lowered from 1,000 to 300, the value Qualys's own QualysETL reference uses and the largest value observed to be honoured without clamping. (3) Defensive exit handling added: missing `hasMoreRecords` field, null `lastSeenAssetId` with `hasMore=1`, and cursor-stall (same `lastSeenAssetId` two pages in a row) all abort with WARNING-level exit-reason logs distinct from clean `hasMore=0` completion. Per-page logging now includes `lastSeenAssetId` and `hasMore` so operators can tail `logs/app.log` and see the cursor advance. New `docs/CSAM_PAGINATION.md` documents the cursor model, defensive exits, page-size rationale, and operator log format. 10 new tests cover the full classification matrix: single page / three pages / cursor stall / >500 pages (proves cap broken) / safety cap with warning / missing hasMoreRecords / null lastSeenAssetId / empty asset[] continues / on_page callback contract / clean completion log format.

## 2026-05-04 — Count-vs-actual drift detection (partial-not-success on short pulls)
**Area:** `src/data_manager.py::_classify_drift`, `src/data_manager.py::refresh_all` (per-API workers), `tests/test_csam_drift.py`, `tests/test_parallel_refresh.py` (fixture defaults)
**Why:** When a refresh's per-API worker returned cleanly but actually pulled fewer rows than the count endpoint reported during preflight (e.g. CSAM pulled 95k of 100k expected, no exception raised — perhaps because of a silent early `hasMore=false` or a quota-related truncation Qualys didn't 429 on), the row was being marked `success` despite the missing 5k assets. Operators had no UI signal that data was still pending. New `_classify_drift(actual, expected, api_label, *, tolerance=0)` helper compares each worker's actual fetch count to its `count_*` preflight expected. Short = `partial` with a clear drift error; positive drift = success (count was stale, benign); `None` expected = success (count endpoint failed, no reference). Crucially the CSAM check queries `csam_assets` by snapshot_fetched_at rather than `len(assets)` — on a resumed pull `assets` only holds this run's new pages while the DB has every page across all resumed runs under the same snapshot, and a `len()` comparison would falsely show partial after every successful resume. VM hosts and detections use `len()` directly (no resume mechanism). 10 new tests cover the helper's classification matrix (short/exact/positive/None/tolerance), end-to-end through `refresh_all` for all three APIs, and the resume-safety case where the DB total matches expected even though `len(assets)` from this run alone does not. Existing test fixture in `test_parallel_refresh.py` updated so the default `count_*` mocks return `None` (drift-bypassed); drift-specific tests override with explicit numeric returns.

## 2026-05-01 — Unbounded streaming CSV exports
**Area:** `src/database.py` (new `iter_latest_*` cursor helpers), `src/data_manager.py::export_csv_stream`, `app.py` (streaming `/api/export/csv`, streaming `/api/data-explorer/export-csv`, new `/api/data-explorer/export-all-zip`, new `_build_export_filters`), `cli.py::cmd_export`, `templates/data_explorer.html`, `tests/test_export_streaming.py`
**Why:** Three operational problems closed in one ticket. (1) Both `cli.py export` and `/api/export/csv` had a hardcoded 100k row cap that silently truncated big-fleet exports — operators got `qualys_detections_*.csv` with the first 100k rows and no warning. Cap removed; new `data_manager.export_csv_stream(...)` yields CSV chunks via `cursor.fetchmany()` so memory stays at ~1k rows regardless of fleet size. (2) `/api/export/csv` accepted `date_from`/`date_to` query params but `app.py`'s hand-built filter dict dropped those keys, so Query Builder's date filters silently no-op'd on export. New `_build_export_filters(args)` helper plumbs them through; `iter_latest_detections` honours them. (3) Operators wanted "all the data in one click" — new `POST /api/data-explorer/export-all-zip` route + Export-all-tables button on Data Explorer streams a ZIP with one CSV per table (csam_assets, vm_hosts, vm_detections, host_tags, detection_changes, health_log). ZIP is built into a tempfile to bound memory; cleaned up via Flask's response-close callback so we don't leak temp files. CLI `python cli.py export --type X` now uses the streaming path too — chunks are written to disk as they arrive and the success line reports approximate row count + file size on disk. 9 new tests cover unbounded export correctness, header-first ordering, the date-filter pass-through bug-fix end-to-end, the data-explorer streaming route, the bulk ZIP shape, and CLI write-through.

## 2026-05-01 — Full-DB purge (with cancel-first orchestration)
**Area:** `src/database.py::purge_all_data`, `src/data_manager.py::purge_all`, `app.py::api_purge_all`, `cli.py` (new `--all`/`--include-config`/`--yes` flags), `templates/settings.html` (Danger Zone card + modal), `static/style.css`, `tests/test_purge_all.py`
**Why:** Existing `purge_daily_snapshots` is retention-based (delete older than N days) — there was no way to wipe the current snapshot or operational state. New `db.purge_all_data(include_config=False)` deletes from every snapshot/derived/operational table; `include_config=True` also wipes asset_owners / sla_targets / saved_queries (full factory reset). Schema is preserved (DELETE not DROP) so the next refresh works immediately, plus a VACUUM to reclaim disk. Crucially, `manager.purge_all()` orchestrates the destructive path: if any refresh is in flight, it sets the cancel event (the one we just added for refresh cancellation) and waits up to 30s for the workers to terminate cleanly before wiping — without this, purging mid-CSAM-pull would yank tables out from under the writer. Three surfaces: `POST /api/purge/all?confirm=YES[&include_config=true]` (with required guard token), Settings page Danger Zone with a two-step "type DELETE" modal confirm, and `python cli.py purge --all` (with `--yes` to skip the interactive prompt for scripts). 9 new tests cover wipe correctness across all 13 tables, the `include_config` toggle, schema preservation post-wipe, the cancel-first orchestration (including the cancel-timeout fallback), and the route's confirm guard.
